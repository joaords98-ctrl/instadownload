/*
  Backend Vercel - IG Media Downloader

  Rotas:
  GET /api/health
  GET /api/instagram/resolve?permalink=...
  GET /api/instagram/download?media_id=...&index=0

  Requisitos no Vercel:
  IG_USER_ID
  IG_ACCESS_TOKEN
  GRAPH_VERSION opcional, exemplo: v21.0
*/
...
function requireConfig() {
  if (!IG_USER_ID || !IG_ACCESS_TOKEN) {
    throw new Error(
      "Backend sem IG_USER_ID ou IG_ACCESS_TOKEN. Configure as variáveis de ambiente no Vercel."
    );
  }
}
...
function absoluteDownloadUrl(req, mediaId, index) {
  ...
  return `${baseUrl}/api/instagram/download?media_id=${encodeURIComponent(
    mediaId
...
async function findMediaByPermalink(permalink) {
  ...
  while (url && page < 15) {
    ...
    for (const item of data) {
      ...
      const samePermalink = target && current && current === target;
      const sameShortcode =
        targetShortcode && currentShortcode === targetShortcode;

      if (samePermalink || sameShortcode) {
        return item;
      }
    }
...
async function filesFromMedia(media, req) {
  ...
  if (media.media_type === "CAROUSEL_ALBUM") {
    ...
    if (!rawFiles.length) {
      rawFiles = await getChildren(media.id);
    }
  } else {
    rawFiles = [media];
  }
...
async function resolveHandler(req, res, parsedUrl) {
  ...
  if (!media) {
    return sendJson(res, 404, {
      ok: false,
      error:
        "Não encontrei esse post/Reel entre as mídias da conta autorizada. Para baixar vídeo sem perder qualidade, a conta conectada precisa ser dona ou ter autorização válida via API."
    });
  }
  ...
  if (!files.length) {
    return sendJson(res, 422, {
      ok: false,
      error:
        "A API encontrou a mídia, mas não retornou media_url baixável. Isso pode acontecer com conteúdo restrito, com copyright/áudio protegido ou permissão insuficiente."
    });
  }
...
async function downloadHandler(req, res, parsedUrl) {
  ...
  if (media.media_type === "CAROUSEL_ALBUM" && !rawFiles.length) {
    rawFiles = await getChildren(media.id);
  }
  ...
  if (!file?.media_url) {
    return sendJson(res, 404, {
      ok: false,
      error: "Arquivo não disponível para download pela API oficial."
    });
  }

  const ext = extensionFor(file);
  const filename = `${slugify(
    media.caption || media.permalink || media.id
  )}-${index + 1}.${ext}`;
  ...
  if (!upstream.ok || !upstream.body) {
    return sendJson(res, upstream.status || 502, {
      ok: false,
      error: "Falha ao baixar arquivo da URL oficial."
    });
  }
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (pathname === "/api/health" || pathname === "/health") {
      return sendJson(res, 200, {
        ok: true,
        configured: Boolean(IG_USER_ID && IG_ACCESS_TOKEN),
        graphVersion: GRAPH_VERSION,
        platform: "vercel"
      });
    }

    if (pathname === "/api/instagram/resolve") {
      return await resolveHandler(req, res, parsedUrl);
    }

    if (pathname === "/api/instagram/download") {
      return await downloadHandler(req, res, parsedUrl);
    }
